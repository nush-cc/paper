...
def evaluate_model(model, test_loader, device, horizon, scaler=None):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            pred_final, pred_base, _, trend, seasonal = model(x)
            ...
            all_trends.append(trend[:, -1, :].cpu().numpy())
            all_seasonals.append(seasonal[:, -1, :].cpu().numpy())

            last_trend = trend.detach().cpu().numpy()
            last_seasonal = seasonal.detach().cpu().numpy()
    
    plot_data["trend"] = last_trend
    plot_data["seasonal"] = last_seasonal
    ...
    full_trend = np.vstack(all_trends)  # [N, C]
    full_seasonal = np.vstack(all_seasonals)  # [N, C]
    ...
    if has_cnn:
        print(
            f"{'Metric':<20} | {'Linear Base':<15} | {'Base + CNN':<15} | {'Improvement':<15}"
        )
        print("-" * 95)
        print(
            f"{'RMSE':<20} | {rmse_b:<15.4f} | {rmse_f:<15.4f} | {rmse_f - rmse_b:+.4f}"
        )
        print(f"{'R2 Score':<20} | {r2_b:<15.4f} | {r2_f:<15.4f} | {r2_f - r2_b:+.4f}")
        print(
            f"{'Avg Accuracy':<20} | {acc_b:<15.4f} | {acc_f:<15.4f} | {acc_f - acc_b:+.4f}"
        )
        print(
            f"{'High Vol Acc':<20} | {h_acc_b:<15.4f} | {h_acc_f:<15.4f} | {h_acc_f - h_acc_b:+.4f}"
        )
    else:
    ...
    print(
        f"{'Sharpe Ratio':<25} | {fin_metrics['Benchmark_Sharpe']:<15.4f} | {fin_metrics['Strategy_Sharpe']:<15.4f} | {diff_sharpe:+.4f}"
    )

    # Cumulative Return
    diff_cum = fin_metrics["Strategy_CumRet"] - fin_metrics["Benchmark_CumRet"]
    print(
        f"{'Cumulative Return':<25} | {fin_metrics['Benchmark_CumRet'] * 100:<14.2f}% | {fin_metrics['Strategy_CumRet'] * 100:<14.2f}% | {diff_cum * 100:+.2f}%"
    )

    # Max Drawdown
    print(
        f"{'Max Drawdown':<25} | {'--':<15} | {fin_metrics['Max_Drawdown'] * 100:<14.2f}% |"
    )
